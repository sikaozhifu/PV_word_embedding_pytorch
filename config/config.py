...
class pc_net:
    ...
    # data_root = osp.join(work_root, 'data', 'ModelNet40', 'point')
    ...
    ckpt_load_file = osp.join(ckpt_folder, 'PCNet-save-ckpt.pth')
    ckpt_argfile = osp.join(ckpt_folder, 'PCNet-save-arg.pth')
...
class view_net:
    ...
    # data_root = osp.join(work_root, 'data', 'ModelNet40', 'view')
    ...
    if base_model_name == (models.ALEXNET or models.RESNET50):
        ...
        ckpt_load_file = osp.join(ckpt_folder,
                                  f'MVCNN-{base_model_name}-save-ckpt.pth')
...
class pv_net:
    ...
    # pc_root = osp.join(work_root, 'data', 'ModelNet40', 'point')
    n_neighbor = 20

    # multi-view cnn
    view_root = osp.join(work_root, 'data', '12_ModelNet40')
    # view_root = osp.join(work_root, 'data', 'ModelNet40', 'view')

    pre_trained_model = False
    ckpt_file = osp.join(ckpt_folder, f'PVNet2-{base_model_name}.pth')
    ckpt_load_file = osp.join(ckpt_folder, f'PVNet2-{base_model_name}.pth')
